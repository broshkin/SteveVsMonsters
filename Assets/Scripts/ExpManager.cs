...
public class ExpManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        RaycastHit hit;
        if (Input.GetMouseButtonDown(0) && Physics.Raycast(ray, out hit, Mathf.Infinity) && hit.transform.gameObject == gameObject)
        {
            MoneySystem.AddMoney(exp_count);
            Destroy(gameObject);
        }

        if (gameObject.transform.position.y < y_offset - 1)
        {
